...
def get_stimulus_control_seen(x: BaseAgent, utterance: str):
    """Returns True if the human has already seen the Stimulus Control Therapy
    instructions."""
    db_entry = DBStimulusControlSeen.objects(user=x.db_user_id).first()
    if db_entry is None:
...
@set_attribute("return_direct", False)
def save_stimulus_control_seen(x: BaseAgent, utterance: str):
    ...
    save_stimulus_control_seen_to_db(x.db_user_id, entry)
